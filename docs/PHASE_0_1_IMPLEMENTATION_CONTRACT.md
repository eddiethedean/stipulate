...
FINAL-001 escalation decision: after following `__wrapped__`, do not infer the selected function's declaring class from a qualified global name, direct class-member identity, or wrapper/property exposure. Those bindings may belong to a replacement class. Use the selected code object's lexical origin to distinguish free functions from class-defined functions; copied `__qualname__` metadata does not establish that origin. A class-defined function's compiler-created `__class__` closure may supply its declaring owner. Without that recorded owner, treat its local namespace as unavailable: name-dependent annotations yield non-permissible `annotation_unresolved`, including on otherwise ordinary decorated methods and properties. Materialized annotations, independent obligations, free-function globals, and explicit requirement `localns` remain usable. This deliberately replaces the earlier expectation that globally exposed decorated methods always have recoverable owner locals. Ordinary declarations inspected directly in their declaring class retain their established owner context. No new public registration API, global class scan, or owner-retaining cache is introduced.
