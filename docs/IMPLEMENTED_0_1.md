...
Wrapped annotations use the selected function's defining owner. If its class-local namespace cannot be recovered, unresolved names remain annotation_unresolved instead of falling back to a conflicting global or builtin. Explicit requirement localns bindings can resolve those names; already materialized types remain usable. Owner selection does not invoke metaclass truthiness.
