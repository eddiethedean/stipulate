...
Wrapped annotations use the selected function's defining context. A class name or a class exposing the function cannot establish its declaring owner: replacement classes may reuse the exact function or any wrapper around it. For class-defined wrapped targets, the compiler-created __class__ closure supplies owner locals when present. Otherwise, name-dependent annotations remain annotation_unresolved and reject in both modes, even for an ordinary decorated method or property; globals and builtins cannot substitute for unknown owner-local precedence. Explicit requirement localns bindings and already materialized types remain usable. Selected free functions use their own globals, identified by their code's lexical origin rather than copied __qualname__ metadata. Ordinary declarations inspected directly in their declaring class retain that context. Owner selection does not execute methods, bind descriptors, scan global classes, or introduce retaining caches.
