...
Wrapped annotations use the selected function's defining owner. A class recovered by qualified name must also expose that exact function through an ordinary method, supported wrapper chain, or standard property accessor; a replacement global class cannot supply unrelated annotation locals. If its class-local namespace cannot be recovered, unresolved names remain annotation_unresolved instead of falling back to a conflicting global or builtin. Explicit requirement localns bindings can resolve those names; already materialized types remain usable. Owner selection does not invoke metaclass truthiness or bind descriptors.
